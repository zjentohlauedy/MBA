...
class LeadersFilter
  ...
  def filter_closers( closers )
    return closers.select { |p| p.innings.to_f >= 0.0 }
  end
end
...
def get_pitcher_stats_by_player_id( player_id )
  result = Utils::transform_hash @db.execute "select player_id, count(1) seasons, sum(wins) wins, sum(losses) losses, sum(games) games, sum(saves) saves, sum(innings) innings, sum(outs) outs, sum(hits) hits, sum(earned_runs) earned_runs, sum(home_runs) home_runs, sum(walks) walks, sum(strike_outs) strike_outs from pitcher_stats_t where player_id = #{player_id} and season_phase = 3 group by player_id"
  result[0]
end

def get_pitcher_stats_by_player_id_and_season( player_id, seasons )
  result = Utils::transform_hash @db.execute "select player_id, count(1) seasons, sum(wins) wins, sum(losses) losses, sum(games) games, sum(saves) saves, sum(innings) innings, sum(outs) outs, sum(hits) hits, sum(earned_runs) earned_runs, sum(home_runs) home_runs, sum(walks) walks, sum(strike_outs) strike_outs from pitcher_stats_t where player_id = #{player_id} and season in #{seasons.to_s.gsub('[','(').gsub(']',')')} and season_phase = 3 group by player_id"
  result[0]
end

def get_batter_stats_by_player_id( player_id )
  result = Utils::transform_hash @db.execute "select player_id, count(1) seasons, sum(games) games, sum(at_bats) at_bats, sum(runs) runs, sum(hits) hits, sum(doubles) doubles, sum(triples) triples, sum(home_runs) home_runs, sum(runs_batted_in) runs_batted_in, sum(walks) walks, sum(strike_outs) strike_outs, sum(steals) steals, sum(errors) errors from batter_stats_t where player_id = #{player_id} and season_phase = 3 group by player_id"
  result[0]
end

def get_batter_stats_by_player_id_and_season( player_id, seasons )
  result = Utils::transform_hash @db.execute "select player_id, count(1) seasons, sum(games) games, sum(at_bats) at_bats, sum(runs) runs, sum(hits) hits, sum(doubles) doubles, sum(triples) triples, sum(home_runs) home_runs, sum(runs_batted_in) runs_batted_in, sum(walks) walks, sum(strike_outs) strike_outs, sum(steals) steals, sum(errors) errors from batter_stats_t where player_id = #{player_id} and season in #{seasons.to_s.gsub('[','(').gsub(']',')')} and season_phase = 3 group by player_id"
  result[0]
end
