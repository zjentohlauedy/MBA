...
def get_pitcher_stats_by_player_id_and_season( player_id, season )
  result = Utils::transform_hash @db.execute "select * from pitcher_stats_t where player_id = #{player_id} and season = #{season} and season_phase = 2"
  result[0]
end

def get_batter_stats_by_player_id_and_season( player_id, season )
  result = Utils::transform_hash @db.execute "select * from batter_stats_t where player_id = #{player_id}  and season = #{season} and season_phase = 2"
  result[0]
end
...
filter   = PlayoffPlayersFilter.new
