...
static int players_t_create_bindings( sqlite3_stmt *statement, const void *data )
{
     ...
     BIND_TXT( statement,  4, player->first_phoenetic );
     BIND_TXT( statement,  5, player->last_phoenetic  );
...
{
     static char query[]   = "INSERT INTO Players_T ( Player_Id, First_Name, Last_Name, First_Phoenetic, Last_Phoenetic, Skin_Tone, Handedness, Player_Type, Rookie_Season, Longevity )"
          /**/                              "VALUES ( ?,"       "?,"        "?,"       "?,"             "?,"            "?,"       "?,"        "?,"         "?,"           "?"       ")";
...
static int players_t_read_retrieve( sqlite3_stmt *statement, const void *data )
{
     ...
     cpytxt( player->first_phoenetic,   sqlite3_column_text( statement, 2 ) );
     cpytxt( player->last_phoenetic,    sqlite3_column_text( statement, 3 ) );
...
{
     static char query[]   = "SELECT First_Name, Last_Name, First_Phoenetic, Last_Phoenetic, Skin_Tone, Handedness, Player_Type, Rookie_Season, Longevity FROM Players_T WHERE Player_Id = ?";
...
static int players_t_update_bindings( sqlite3_stmt *statement, const void *data )
{
     ...
     BIND_TXT( statement,  3, player->first_phoenetic );
     BIND_TXT( statement,  4, player->last_phoenetic  );
