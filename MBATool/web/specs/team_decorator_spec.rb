...
describe TeamDecorator do
  ...
  describe '#decorate_team' do
    it 'should return the given team hash with links added [deprecated]' do
    ...
    it 'should add a self link [deprecated]' do
      ...
      expect( link[:href] ).to     eq     "#{@base_url}/teams/1"
    end

    it 'should add a players link [deprecated]' do
      team = {team_id: 1, name: 'Name1', location: 'Location1', primary_color: 5, secondary_color: 2}

      result = @team_decorator.decorate_team team

      link = find_link result[:links], 'players'

      expect( link        ).to_not be_nil
      expect( link[:rel ] ).to     eq     'players'
      expect( link[:href] ).to     eq     "#{@base_url}/teams/1/players"
    end
    ...
    it 'should add a stats link [deprecated]' do
      ...
      expect( link[:href] ).to     eq     "#{@base_url}/teams/1/stats"
    end
  ...
  describe '#decorate_team_stats' do
    ...
    it 'should add a self link [deprecated]' do
      ...
      expect( link        ).to_not be_nil
      expect( link[:rel ] ).to     eq     'self'
      expect( link[:href] ).to     eq     "#{@base_url}/teams/1/stats?season=1&phase=#{Phases::RegularSeason}"
    end
