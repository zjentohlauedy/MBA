...
static int pitcher_stats_t_update_bindings( sqlite3_stmt *statement, const void *data )
{
     ...
     BIND_DBL(  statement,  5, pitcher_stats->innings     );
     BIND_INT(  statement,  6, pitcher_stats->hits        );
     BIND_INT(  statement,  7, pitcher_stats->earned_runs );
     BIND_INT(  statement,  8, pitcher_stats->home_runs   );
     BIND_INT(  statement,  9, pitcher_stats->walks       );
     BIND_INT(  statement, 10, pitcher_stats->strike_outs );

     BIND_INT(  statement, 11, pitcher_stats->player_id    );
     BIND_INT(  statement, 12, pitcher_stats->season       );
     BIND_INT(  statement, 13, pitcher_stats->season_phase );
...
int pitcher_stats_t_update( sqlite3 *db, const pitcher_stats_s *pitcher_stats )
{
     ...
     return execute_query( db, query, pitcher_stats_t_update_bindings, pitcher_stats, NULL, NULL );
}
...
{
     return execute_query( db, "DELETE FROM Pitcher_Stats_T WHERE Player_Id = ? AND Season = ? AND Season_Phase = ?", pitcher_stats_t_delete_bindings, pitcher_stats, NULL, NULL );
}
