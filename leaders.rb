...
class Pitcher < Stats
  ...
  def initialize( str, team )
    ...
    @pos          = fields[ 0]
    @wins         = fields[ 7].to_i
    @losses       = fields[ 8].to_i
    @era          = fields[ 9].to_f
    @games        = fields[10].to_i
    @saves        = fields[11].to_i
    @innings      = fields[12].to_f
    @hits         = fields[13].to_i
    @earned_runs  = fields[14].to_i
    @home_runs    = fields[15].to_i
    @walks        = fields[16].to_i
    @strike_outs  = fields[17].to_i
    @vsba         = fields[18].to_f
    @inn_per_game = fields[19].to_f
    @hits_per_9   = fields[20].to_f
    @walks_per_9  = fields[21].to_f
    @so_per_9     = fields[22].to_f
    @hr_per_9     = fields[23].to_f
...
class Batter < Stats
  ...
  def initialize( str, team )
    ...
    @team        = team
    @pos         = fields[ 0]
    @average     = fields[ 9].to_f
    @games       = fields[10].to_i
    @at_bats     = fields[11].to_i
    @runs        = fields[12].to_i
    @hits        = fields[13].to_i
    @doubles     = fields[14].to_i
    @triples     = fields[15].to_i
    @home_runs   = fields[16].to_i
    @rbi         = fields[17].to_i
    @walks       = fields[18].to_i
    @strike_outs = fields[19].to_i
    @steals      = fields[20].to_i
    @errors      = fields[21].to_i
    @slugging    = fields[22].to_f
    @obp         = fields[23].to_f
    @soa         = fields[24].to_f
    @rpg         = fields[25].to_f
  end
